...
代码结构也遵循这条边界：`devloop/domain/` 持有 Workspace / Repo / Component、branch/PR 状态及合法变化，`devloop/lib/` 提供 Git/forge/ecosystem/config 等技术能力；`devloop/hooks/` 和 `devloop/scripts/` 分别把工具事件与工作流动作驱动到领域层，让 LLM 对 workspace/repo 的修改经过可控入口。
...
devloop 拥有 coding harness 内部的开发小闭环：分支开发、验证、commit/push、创建 PR/MR、
产出 review 以及执行级护栏。它会留下可持续观察的开发事实，但不拥有 Requirement 身份、跨仓
编排、长期调度或会话唤醒；这些职责属于 Baton 及其
[ReqLoop Marketplace](https://github.com/compforge/reqloop)。ReqLoop 可以观察 devloop 的产出，
但 devloop 不再为需求级闭环建立第二套控制面。
