...
- **状态 reconciliation**：Forge 当前状态是事实，`pull_request_lifecycle` 把它映射为幂等动作，不依赖捕获某次边沿事件。`merged + managed worktree` 会尝试回收 checkout；dirty、任意 harness 的活跃 owner、当前 checkout 均延后重试；primary / external checkout 与 closed-but-unmerged branch 不自动删除。branch ref 始终保留。
