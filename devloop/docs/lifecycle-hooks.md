...
`make test TEST_FILES="..."` 执行改动测试。没有改动测试时回退全量；已有改动测试但项目未采用
该 Make 契约时回退全量，并在 lifecycle PLAN 中提示补充 `TEST_FILES` 支持；该 guidance 不参与
gate 的 `ok` / `proceed`，不会阻断 commit 或 MR。
