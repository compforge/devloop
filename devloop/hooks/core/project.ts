...
import type { Change, CommandTarget, FileChangeTarget, Target, WorkingDirectory } from "./domain.js";
...
function splitShell(command: string): readonly string[] {
  const parts: string[] = [];
  let current = ""; let quote = ""; let escaped = false; let depth = 0;
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (depth === 0 && (pair === "&&" || pair === "||")) { if (current.trim()) parts.push(current.trim()); current = ""; index += 1; continue; }
    if (depth === 0 && (char === ";" || char === "\n" || char === "|")) { if (current.trim()) parts.push(current.trim()); current = ""; continue; }
    current += char;
  }
...
function shellWords(command: string): readonly string[] {
  ...
  for (const char of command.trim()) {
    ...
    if (/\s/.test(char) || "(){}".includes(char)) { if (current) { words.push(current); current = ""; } continue; }
    current += char;
  }
...
function commandTarget(words: readonly string[], base: WorkingDirectory): CommandTarget | undefined {
  ...
  if (["git", "go", "make"].includes(basename(argv[0]!))) {
    ...
    if (index >= 0 && argv[index + 1]) {
      ...
      if (basename(argv[0]!) === "git") argv = [...argv.slice(0, index), ...argv.slice(index + 2)];
    }
  }
  ...
  if (basename(argv[0]!) === "git") {
    ...
    subcommand = argv[index]; args = argv.slice(index + 1);
  }
  return { kind: "command", argv, workingDirectory, environment, ...(subcommand ? { subcommand } : {}), args, ...(dashC ? { dashC } : {}) };
}

function commandTargets(command: string, base: WorkingDirectory): readonly Target[] {
...
function commandSubstitutions(source: string): readonly string[] {
  const values: string[] = [];
  ...
  for (let index = 0; index < source.length - 1; index += 1) {
    ...
    if (char === "$" && source[index + 1] === "(") {
      ...
      if (end > index) { values.push(source.slice(index + 2, end)); index = end; }
    }
  }
  return values;
}

export function patchFileChanges(value: unknown): readonly FileChangeTarget[] {
  if (typeof value !== "string") return [];
  return value.split("\n").flatMap((line): FileChangeTarget[] => {
    for (const [prefix, mode] of [["*** Add File: ", "write"], ["*** Update File: ", "edit"], ["*** Delete File: ", "edit"]] as const) {
...
function execTargets(toolInput: Record<string, unknown>, cwd: string): readonly Target[] {
  ...
  for (const match of source.matchAll(calls)) {
    try {
      ...
      if (typeof row.cmd !== "string") continue;
  ...
  if (source.includes("tools.apply_patch")) {
    ...
    for (const match of source.matchAll(/"(?:\\.|[^"\\])*"/g)) {
      try {
        ...
        if (typeof decoded !== "string" || !decoded.startsWith("*** Begin Patch") || seen.has(decoded)) continue;
        seen.add(decoded); targets.push(...patchFileChanges(decoded));
...
export function projectTool(input: HarnessToolInput): Change {
  const { toolName, toolInput, cwd } = input;
  if (toolName === "exec") return { targets: execTargets(toolInput, cwd), cwd, tool: toolName, command: "" };
  ...
  if (FILE_TOOLS.has(toolName)) {
    if (toolName === "str_replace_editor" && toolInput.command === "view") return { targets: [], cwd, tool: toolName, command: "" };
