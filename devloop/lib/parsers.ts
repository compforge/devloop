...
export interface ReferenceEntry { readonly title: string; readonly path: string; readonly description: string }
...
function concretePath(value: string, base: string): string {
  if (value.includes("<") || value.includes(">")) return value;
  const expanded = value.startsWith("~") ? `${process.env.HOME ?? ""}${value.slice(1)}` : value;
  return isAbsolute(expanded) ? expanded : resolve(base, expanded);
}

export function parseReferencesSection(path: string): readonly ReferenceEntry[] {
...
export function parseSubprojectsSection(path: string): readonly SubprojectEntry[] {
  const content = read(path);
  if (content === undefined) return [];
  ...
  if (lines.length < 3 || lines[0] === undefined) return [];
  const headers = lines[0].trim().replace(/^\||\|$/g, "").split("|").map((cell) => cell.trim());
  return lines.slice(2).flatMap((line): SubprojectEntry[] => {
    ...
    const entry: { name: string; path: string; aliases: string[]; language?: string; role?: string; note?: string } = {
