...
# `test.json`）；而 component worker 不写文件（test 并行 fan-out 在 join 后由父线程批量落盘），故
# component 是段**内**的 JSON key，不是目录——拆成目录既不解决那个 race，component id 又不是安全路径
# 分量（根 component 是 `.`、可含斜杠），当目录还得枚举目录才能读回全集。
#
