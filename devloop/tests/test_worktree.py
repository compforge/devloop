...
def test_reconciliation_never_removes_the_checkout_it_runs_from():
    ...
    assert [(action.pr_number, action.status, action.reason) for action in actions] == [
        (1, "deferred", "current_checkout")
    ]


def test_merged_pr_reconciliation_removes_only_safe_managed_worktrees():
    repo, _, _ = _fixture("merged_reconcile")
    ...
    for retained in (repo, dirty, owned, open_path, closed, external):
        assert Path(retained).is_dir()
    assert [(a.pr_number, a.status, a.reason) for a in actions] == [
        (2, "completed", "removed"),
        (3, "deferred", "dirty"),
        (4, "deferred", "active_owner"),
    ]
